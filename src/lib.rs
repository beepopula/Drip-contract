...
const RESOLVE_COLLECT_DRIP_GAS: u64 = 20_000_000_000_000;
...
impl Contract {
    ...
    #[init(ignore_state)]
    pub fn migrate() -> Self {
        let old_contract: OldContract = env::state_read().unwrap();
        assert!(env::predecessor_account_id() == old_contract.owner_id, "not owner");
        env::storage_remove("STATE".as_bytes());
        Self {
    ...
    pub fn ft_collect(&mut self, collects: Vec<AccountId>) {
        ...
        assert!(collects.len() as u64 * COLLECT_DRIP_GAS + RESOLVE_COLLECT_DRIP_GAS < (env::prepaid_gas() - env::used_gas()).0, "not enough gas");
        ...
        }).to_string().as_bytes(), 0, RESOLVE_COLLECT_DRIP_GAS.into());
